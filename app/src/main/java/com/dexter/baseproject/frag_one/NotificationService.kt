...
class NotificationService : IntentService("MyService") {
    ...
    override fun onHandleIntent(p0: Intent?) {
        ...
        val notification = NotificationCompat.Builder(applicationContext,
            ForegroundWorker.channelId
        )
            .setSmallIcon(R.mipmap.ic_launcher)
            .setContentTitle("Important background job")
            .build()
        startForeground(Worker.NOTIFICATION_ID, notification)
    ...
    private fun showProgress(
        ...
        val notification = NotificationCompat.Builder(applicationContext, Worker.channelId)
            .setSmallIcon(R.mipmap.ic_launcher)
            .setContentTitle("Important background job")
    ...
    private fun createNotificationChannel(notificationManager: NotificationManager) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            var notificationChannel =
                notificationManager?.getNotificationChannel(Worker.channelId)
            if (notificationChannel == null) {
                notificationChannel = NotificationChannel(
                    Worker.channelId, Worker.TAG, NotificationManager.IMPORTANCE_HIGH
                )
